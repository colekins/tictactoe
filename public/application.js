...
var gameBoard = ['d0', 'd1', 'd2', 'd3', 'd4', 'd5', 'd6', 'd7', 'd8'];
var lastTurn = 'O';

// each tictactoe square:
var divZero = document.querySelector('#d0');
var divOne = document.querySelector('#d1');
var divTwo = document.querySelector('#d2');
var divThree = document.querySelector('#d3');
var divFour = document.querySelector('#d4');
var divFive = document.querySelector('#d5');
var divSix = document.querySelector('#d6');
var divSeven = document.querySelector('#d7');
var divEight = document.querySelector('#d8');
// const gameSquare = document.querySelector('.col-xs-4')

var indexNum = '';
var updateBoard = function updateBoard(divId) {
...
var checkBoard = function checkBoard() {
  var xOrO = function xOrO(element) {
...
var reset = function reset() {
  ...
  for (var i = 0; i < gameBoard.length; i++) {
    var current = document.querySelector('#d' + i);
...
var resetNoUser = function resetNoUser() {
  gameBoard = ['d0', 'd1', 'd2', 'd3', 'd4', 'd5', 'd6', 'd7', 'd8'];
  lastTurn = 'O';
  for (var i = 0; i < gameBoard.length; i++) {
    var current = document.querySelector('#d' + i);
...
var getGames = function getGames(data) {
  api.index(data).then(ui.onGetGameSuccess);
};
...
var updateGame = function updateGame(indexNum, lastTurn) {
  var data = {
    game: {
      cell: {
        index: '',
  ...
  api.update(data).then(ui.onCreateSuccess);
};
var finishGame = function finishGame(indexNum, lastTurn) {
  var data = {
    game: {
      cell: {
        index: '',
...
var onSignUp = function onSignUp(event) {
  ...
  api.signUp(data).then(ui.signUpSuccess).then(document.getElementById('sign-up').reset()).then(hideSignUp).catch(ui.signUpFailure);
};

var onSignIn = function onSignIn(event) {
  var data = getFormFields(this);
  event.preventDefault();
  api.signIn(data).then(ui.signInSuccess).then(unhideSignOut).then(hideSignIn)
  // .then(getGames)
  .then(unhideGames).then(showPassBtn).then(hideSignUpBtn).then(newGame).then(showGame).catch(ui.signInFailure);
};
...
var onChangePassword = function onChangePassword(event) {
  var data = getFormFields(this);
  event.preventDefault();
  api.changePassword(data).then(ui.passwordSuccess).then(document.getElementById('change-password').reset()).then(hidePass).catch(ui.passwordFail);
};

var onSignOut = function onSignOut(event) {
  event.preventDefault();
  api.signOut().then(ui.signOutSuccess).then(hideSignIn).then(unhideSignOut).then(unhideGames).then(showPassBtn).then(hideSignUpBtn).then(showGame).then(document.getElementById('sign-in').reset()).catch(ui.signOutFail);
};

var unhide = function unhide() {
...
var unhideSignOut = function unhideSignOut() {
  $('.signOutButton').toggleClass('hidden unhidden');
  // console.log(parentElement)
};
var unhideGames = function unhideGames() {
...
var hidePass = function hidePass() {
  $('.passForm').toggleClass('hidden unhidden');
  // console.log(parentElement)
};

var showPassBtn = function showPassBtn() {
  $('.passBtn').toggleClass('hidden unhidden');
  // console.log(parentElement)
};

var hideSignUpBtn = function hideSignUpBtn() {
...
var indexGame = function indexGame(value) {
  return $.ajax({
